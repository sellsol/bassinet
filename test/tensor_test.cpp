...
TEST(TensorIntlTest, Construct) {
    bassinet::TensorIntl t({2, 3}, 5.0f);
    EXPECT_EQ(t.shape(), std::vector<size_t>({2, 3}));
    EXPECT_EQ(t.size(), 6);
}
TEST(TensorIntlTest, ConstructDefault) {
    bassinet::TensorIntl t({2, 2}, 7.5f);
...
TEST(TensorIntlTest, Transpose3D) {
    bassinet::TensorIntl t({2, 3, 4}, 0.0f);
    for (size_t i = 0; i < 2; ++i)
...
TEST(TensorTest, ConstructDefault) {
    ...
    for (size_t i = 0; i < 2; ++i) {
        for (size_t j = 0; j < 2; ++j) {
            EXPECT_FLOAT_EQ(t.intl->at({i, j}), 7.5f);
        }
    }
}
...
TEST(TensorTest, Matmul1Dx1D) {
    ...
    a.intl->at({0}) = 1.0f; a.intl->at({1}) = 2.0f; a.intl->at({2}) = 3.0f;

    bassinet::Tensor b({3}, 0.0f);
    b.intl->at({0}) = 4.0f; b.intl->at({1}) = 5.0f; b.intl->at({2}) = 6.0f;
...
TEST(TensorTest, Matmul2Dx2D) {
    ...
    a.intl->at({0,0}) = 1; a.intl->at({0,1}) = 2; a.intl->at({0,2}) = 3;
    a.intl->at({1,0}) = 4; a.intl->at({1,1}) = 5; a.intl->at({1,2}) = 6;

    bassinet::Tensor b({3, 2}, 0.0f);
    b.intl->at({0,0}) = 7;  b.intl->at({0,1}) = 8;
    b.intl->at({1,0}) = 9;  b.intl->at({1,1}) = 10;
    b.intl->at({2,0}) = 11; b.intl->at({2,1}) = 12;
...
TEST(TensorTest, Matmul1Dx2D) {
    bassinet::Tensor a({3}, 0.0f);
    a.intl->at({0}) = 1; a.intl->at({1}) = 2; a.intl->at({2}) = 3;

    bassinet::Tensor b({3, 2}, 0.0f);
    b.intl->at({0,0}) = 4; b.intl->at({0,1}) = 5;
    b.intl->at({1,0}) = 6; b.intl->at({1,1}) = 7;
    b.intl->at({2,0}) = 8; b.intl->at({2,1}) = 9;
...
TEST(TensorTest, Matmul2Dx1D) {
    ...
    bassinet::Tensor b({3}, 0.0f);
    b.intl->at({0}) = 7; b.intl->at({1}) = 8; b.intl->at({2}) = 9;
...
TEST(TensorTest, MatmulBatched3Dx3D) {
    ...
    // batch 0
    a.intl->at({0,0,0}) = 0; a.intl->at({0,0,1}) = 2;
    a.intl->at({0,1,0}) = -2; a.intl->at({0,1,1}) = -5;
    // batch 1
    a.intl->at({1,0,0}) = -5; a.intl->at({1,0,1}) = -5;
    a.intl->at({1,1,0}) = -1; a.intl->at({1,1,1}) = 2;

    bassinet::Tensor b({2, 2, 2}, 0.0f);
    // batch 0
    b.intl->at({0,0,0}) = 6; b.intl->at({0,0,1}) = -6;
    b.intl->at({0,1,0}) = 3; b.intl->at({0,1,1}) = 0;
    // batch 1
    b.intl->at({1,0,0}) = -2; b.intl->at({1,0,1}) = -3;
    b.intl->at({1,1,0}) = 3; b.intl->at({1,1,1}) = 5;
    ...
    for (size_t i = 0; i < 2; ++i)
        for (size_t j = 0; j < 3; ++j)
            for (size_t k = 0; k < 5; ++k)
                EXPECT_FLOAT_EQ(result.intl->at({i, j, k}), 8.0f);
}
...
TEST(TensorTest, MatmulBroadcastBatchRight) {
    ...
    for (size_t i = 0; i < 2; ++i)
        for (size_t j = 0; j < 4; ++j)
            for (size_t k = 0; k < 5; ++k)
                EXPECT_FLOAT_EQ(result.intl->at({i, j, k}), 6.0f);
}
